...
import com.azure.core.rest.PagedFlux;
...
public class SearchIndexerAsyncClient {
    ...
    public Mono<Void> deleteDataSourceConnection(String dataSourceName) {
        return withContext(context -> deleteDataSourceConnectionWithResponse(dataSourceName, null, context)
            .flatMap(SearchIndexerAsyncClient::toMono));
    }
    ...
    public Mono<Void> resetIndexer(String indexerName) {
        return resetIndexerWithResponse(indexerName).flatMap(SearchIndexerAsyncClient::toMono);
    }
    ...
    public Mono<Void> runIndexer(String indexerName) {
        return runIndexerWithResponse(indexerName).flatMap(SearchIndexerAsyncClient::toMono);
    }
    ...
    private static <T> com.azure.core.rest.PagedFlux<T> pagedFluxError(ClientLogger logger, RuntimeException ex) {
        return new com.azure.core.rest.PagedFlux<>(() -> monoError(logger, ex));
    }
     ...
     * @param response whose {@link com.azure.core.http.rest.Response#getValue() value} is to be converted
     * @return The converted {@link Mono}
     */
    private static <T> Mono<T> toMono(com.azure.core.rest.Response<T> response) {
