...
public final class SearchAsyncClient {
    ...
    private static final SerdeAdapter SERDER = initializeSerdeAdapter();
        ...
        try {
            return restClient.getDocuments()
                ...
                .map(res -> {
                    if (serializer == null) {
                        try {
                            String serializedJson = SERDER.serialize(res.getValue(), SerdeEncoding.JSON);
                            T document = SERDER.deserialize(serializedJson, modelClass, SerdeEncoding.JSON);
