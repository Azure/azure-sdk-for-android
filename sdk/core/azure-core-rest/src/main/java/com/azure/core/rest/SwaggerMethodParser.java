...
import com.azure.core.rest.annotation.FormParam;
...
import com.azure.core.rest.annotation.Headers;
import com.azure.core.rest.annotation.HostParam;
import com.azure.core.rest.annotation.Patch;
...
//     * @param rawHost the raw host value from the @Host annotation. Before this can be used as the host value in an
//     * HTTP request, it must be processed through the possible host substitutions.
//     */
//    SwaggerMethodParser(Method swaggerMethod, String rawHost) {
//        this(swaggerMethod, rawHost, JacksonSerderAdapter.createDefaultSerdeAdapter());
//    }

    /**
     * Creates SwaggerMethodParser.
     *
    ...
    public void setEncodedQueryParameters(Object[] swaggerMethodArguments, UrlBuilder urlBuilder) {
        ...
        for (Substitution substitution : querySubstitutions) {
            ...
            if (0 <= parameterIndex && parameterIndex < swaggerMethodArguments.length) {
                ...
                String parameterValue = serialize(serdeAdapter, methodArgument);
            ...
            if (0 <= parameterIndex && parameterIndex < swaggerMethodArguments.length) {
                ...
                if (methodArgument instanceof Map) {
                    ...
                    for (final Map.Entry<String, ?> headerCollectionEntry : headerCollection.entrySet()) {
                        ...
                        final String headerValue = serialize(serdeAdapter, headerCollectionEntry.getValue());
                        if (headerValue != null) {
                    ...
                    final String headerValue = serialize(serdeAdapter, methodArgument);
                    if (headerValue != null) {
                        httpHeaders.put(headerName, headerValue);
                    }
                }
    ...
    public Object setBody(Object[] swaggerMethodArguments) {
        ...
        if (!CoreUtils.isNullOrEmpty(formSubstitutions) && swaggerMethodArguments != null) {
            result = formSubstitutions.stream()
                .map(substitution -> serializeFormData(serdeAdapter, substitution.getUrlParameterName(),
    ...
    private static String serialize(SerdeAdapter serdeAdapter, Object value) {
        if (value == null) {
            return null;
        }
        ...
        if (value instanceof String) {
            ...
            return serdeAdapter.serializeRaw(value);
        }
    }

    private static String serializeFormData(SerdeAdapter serdeAdapter, String key, Object value,
        ...
        if (value instanceof List<?>) {
            return ((List<?>) value).stream()
                .map(element -> serializeAndEncodeFormValue(serdeAdapter, element, shouldEncode))
                .filter(Objects::nonNull)
                .map(formValue -> encodedKey + "=" + formValue)
                .collect(Collectors.joining("&"));
        } else {
            return encodedKey + "=" + serializeAndEncodeFormValue(serdeAdapter, value, shouldEncode);
        }
    }

    private static String serializeAndEncodeFormValue(SerdeAdapter serializer, Object value,
                                                      boolean shouldEncode) {
        if (value == null) {
            return null;
        }
        ...
        if (methodArguments != null) {
            for (Substitution substitution : substitutions) {
                ...
                if (0 <= substitutionParameterIndex && substitutionParameterIndex < methodArguments.length) {
                    ...
                    String substitutionValue = serialize(serdeAdapter, methodArgument);
