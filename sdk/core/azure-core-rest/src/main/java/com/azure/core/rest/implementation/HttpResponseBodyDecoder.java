...
public final class HttpResponseBodyDecoder {
    ...
    public static Mono<Object> decode(final String body,
        ...
        return decodeByteArray(body == null ? null : body.getBytes(StandardCharsets.UTF_8),
            httpResponse, serdeAdapter, decodeData);
    }
                ...
                return bodyMono.flatMap(bodyAsByteArray -> {
                    try {
                        final Object decodedErrorEntity = deserializeBody(bodyAsByteArray,
                            ...
                            null, serdeAdapter, SerdeEncoding.fromHeaders(httpResponse.getHeaders().toMap()));
    ...
    private static Object deserializeBody(final byte[] value, final Type resultType, final Type wireType,
                                          final SerdeAdapter serdeAdapter,
                                          final SerdeEncoding encoding) throws IOException {
        InputStream inputStream = (value == null || value.length == 0)
            ? null
            : new ByteArrayInputStream(value);

        if (wireType == null) {
            return serdeAdapter.deserialize(inputStream, resultType, encoding);
        } else if (TypeUtil.isTypeOrSubTypeOf(wireType, Page.class)) {
            return deserializePage(inputStream, resultType, wireType, serdeAdapter, encoding);
        } else {
            final Type wireResponseType = constructWireResponseType(resultType, wireType);
            final Object wireResponse = serdeAdapter.deserialize(inputStream, wireResponseType, encoding);
    ...
    private static boolean isReturnTypeDecodable(Type returnType) {
        ...
        return !isFluxByteBuffer(returnType)
